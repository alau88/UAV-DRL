...
class UAVPerformanceMetrics:
    ...
    def update(self, uav_positions, user_positions, total_rewards):
        ...
        coverage_counts = [np.any(np.linalg.norm(user_positions - uav_pos, axis=1) < self.coverage_threshold) for
                           uav_pos in uav_positions]
        
        self.num_users_covered += np.sum(coverage_counts)
    ...
    def get_metrics(self):
        ...
        average_coverage_per_step = self.num_users_covered / self.steps if self.steps > 0 else 0
        ...
        return {
            ...
            'num_users_covered': self.num_users_covered,
